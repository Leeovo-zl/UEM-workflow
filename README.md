...
The `demo/` folder contains a small simulated image sequence covering one period of the motion. The `docs/figures/` folder contains representative visual outputs used by this README. Other generated outputs are created by the scripts and are not tracked in the repository.
...
Intermediate PNG outputs are generated by Steps 1-7 and can be used to visually inspect image enhancement, centerline extraction, masking, curve fitting, overlay quality, centroid displacement, and dark spot detection. The expected output filenames are grouped below by workflow step. The embedded images are representative examples stored in `docs/figures/` for README display.
...
- `sharp.png`
  ...
  ![Enhanced sharp image](docs/figures/sharp.png)
...
- `overlay_centerlines_ridge.png`
  ...
  ![Centerline overlay](docs/figures/overlay_centerlines_ridge.png)
...
- `snake_fitted_curve_thick.png`
  ...
  ![Fitted curve mask](docs/figures/snake_fitted_curve_thick.png)
...
Step 8 produces CSV/TXT summaries only in this cleaned workflow, so no PNG representative image is generated for this step.
